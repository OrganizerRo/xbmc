...
## Architecture Decision: Win32 (x86) Only
...
**Decision**: **Win32 (x86) only** for the Nexus workflow, matching Leia.
`0_package.target-x64.list` is present but is missing several packages that
Nexus CMake requires and that exist only in the win32 list (giflib,
libjpeg-turbo, libpng, libcec, shairplay, fstrcmp, dnssd, detours, libplist).
Adding x64 without first populating those packages would cause CMake configure
to fail; x64 is therefore deferred to **Future Work** (item 1 below).
...
9. [cache miss] vcvarsall.bat x86 + bash --login make-mingwlibs.sh --prompt=noprompt --mode=clean --win10=no --build32=yes
...
15. vcvarsall.bat x86 + bootstrap-addons.bat
...
19. vcvarsall.bat x86 + build each addon individually (with per-addon logs)
...
|----------|--------|-----------|
| Architecture | Win32 (x86) only | Matches Leia; all required pre-built packages present in win32 list; x64 list missing giflib/libjpeg-turbo/libpng/libcec/etc. |
| CMake generator | Visual Studio 17 2022 | On windows-2022 runner, VS 2022 is pre-installed |
| FFmpeg build | make-mingwlibs.sh `--build32=yes --win10=no` | Produces MSVC .lib via `--toolchain=msvc`; `--win10=no` sets `_WIN32_WINNT=0x0600` and `NTDDI_VERSION` to match local `make-mingwlibs.bat` defaults |
| vcvarsall | `vcvarsall.bat x86` (not vcvars32.bat) | `vcvarsall.bat <arch>` is the arch-parameterised entrypoint; `vcvars32.bat` is a fixed-x86 wrapper |
...
1. **x64 variant** — before adding `-A x64` + `--build64=yes`, populate
   `0_package.target-x64.list` with the packages currently missing vs the
   win32 list: giflib, libjpeg-turbo, libpng, libcec, shairplay, fstrcmp,
   dnssd, detours, libplist.  Only then will CMake configure succeed for x64.
